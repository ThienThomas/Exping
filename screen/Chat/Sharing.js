...
import { getDocList, getImageList, getVidList } from '../../mymodules/CompressImg';
const Tab = createMaterialTopTabNavigator()
export default function Sharing({route}) {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
        const res = await getImageList(userinfo.email)
        setImageList(res)
      }, 1000)
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
        const res = await getVidList(userinfo.email)
        setVideoList(res)
      }, 1000)
      return () => clearInterval(interval)
  },[])
  useEffect(() => {
    const interval = setInterval(async () => {
        const res = await getDocList(userinfo.email)
        setDocsList(res)
      }, 1000)
      return () => clearInterval(interval)
  },[])
