...
import { CreateGRoup } from "../../mymodules/CompressImg";
...
export default function CreateGroupInfo({route}){
    ...
    async function CreateGroupInformation(){
        return await CreateGRoup(users, displayName, selectedImage || "none")
        // Alert.alert('Thành công !', displayName, [
    ...
    return (
        ...
        <Modal
            ...
            <View style={{
                ...
                <Pressable  
                    onPress={async () => {
                        const res = await CreateGroupInformation()
                        if (res === 'created'){
        ...
        {isloading ? <AppLoadingAnimation></AppLoadingAnimation> : null}
        </>
    )
}