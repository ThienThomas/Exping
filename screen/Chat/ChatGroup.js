...
export default function ChatGroup({route}) {
    ...
    async function handlePickPhoto(){
        ...
        if (!result.cancelled){
            if (result.type == 'image') {
                navigation.navigate('pickphotogroup', {image: result, groupinfo: groupinfo, groupid: groupid})
            }
            else {
                navigation.navigate('pickvideogroup', {video: result, groupinfo: groupinfo, groupid: groupid})
            }
          } 
      }
    ...
    const fecthMess = async () => {
        const result = await GetGroupMessages(route.params.groupinfo.docid)
    ...
    const onSend = async () =>{
        if (mess){
          ...
          const result = await axios({
              ...
              url: `${baseurl}/group/sendgroupmess`
          }).then((response) => {
        ...
        <View style={{flex: 1, backgroundColor: 'white', paddingBottom: 10}}>
          <GiftedChat
              ...
              renderAvatar={(props) => {
                  return (
                    <FriendsAvatar
                    Img={props.currentMessage.sender.photoURL}
                     Width={36}
                     Height={36} 
                   />
                  )
                  ...
                  options={{
                    ['Máy ảnh']:  async () => { 
                        if(Camera.requestCameraPermissionsAsync()){
                          navigation.navigate('takephotogroup', {groupid: groupid})
                        }
                      },
                    ['Video'] : () => {
